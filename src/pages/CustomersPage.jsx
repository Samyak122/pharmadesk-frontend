            ...
            {history.invoices?.length ? (
              <div className="space-y-2">
                {history.invoices.map((invoice) => (
                  <div key={invoice.invoice_id} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600 shadow-sm">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="font-semibold text-slate-900">{invoice.invoice_no}</div>
                        <div className="mt-1 text-slate-500">{invoice.invoice_date}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-semibold text-slate-900">{formatCurrency(invoice.total_amount)}</div>
                        <div className="mt-1 text-slate-500">{invoice.payment_status || 'Pending'}</div>
                      </div>
                    </div>
                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                        <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Payment Status</p>
                        <p className="mt-1 font-medium text-slate-700">{invoice.payment_status || '—'}</p>
                      </div>
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                        <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Payment Method</p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button type="button" title="Editing finalized invoices is not available." className="cursor-not-allowed rounded-2xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm font-medium text-slate-400" disabled>
                        Edit Bill
                      </button>
                    </div>
                  </div>
